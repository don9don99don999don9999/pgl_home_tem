...
def index(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            #genes11=[]
            for gene in genes: 
                network1 = set(AI1.objects.filter(source=gene)|AI1.objects.filter(target=gene)) 
                edges1   = [[x.source, x.target] for x in network1]
                edges += edges1
                #gene11 = '#%s'%gene
                #genes11.append(gene11)
            gene11 =[]
            for x in genes:
                if x =='':
                    pass
...
#           return render(request, 'network_test/index.html',{'form': form,'query':','.join(genes)}) # 렌덤으로 가져온 클레스를 html로 보낸다. 
            return render(request, 'network_test/index.html',{'jsonf':jsonfilename.split('/')[-1],'form':form,'query':(', '.join(gene11))})
